...
class State(TypedDict):
    #tool : 함수호출로 해결 가능하면 tool
    #agent : 판단이 필요하면 llm 호출하는 agent
    user_input : str #사용자의 현재 상황과 질문 - agent
    location : str   #사용자의 지역 정의 - tool
    time_slot : str #질문 시간대(아침, 점심, 저녁...) - tool
    season :str      #현재 계절 - tool
    weather : str    #현재 날씨 - tool
    intent : str     #사용자에게 추천할 카테고리 의도(food, activity,unknown) - agent
    
    recommend_items : list # 추천된 음식, 활동의 리스트
    search_keyword : str   # 장소 검색용 키워드
    recommend_place : dict # 장소 추천 결과(장소이름, 주소, url)
...
#search_place : 실제로 만든 키워드로 '카카오 맵'에 장소 검색
