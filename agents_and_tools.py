...
def recommend_activity(state:dict) -> dict :
    ...
    #[[1,2,3], [2,3,4]] -> 1차원으로 평탄화 [1,2,3,2,3,4]
    #itertools
    if isinstance(items, dict):
        items = [i for sub in items.values() for i in (sub if isinstance(sub, list) else [sub])]
    ...
    #이 함수의 최종 목표 : 추천활동을 리스트로 만들어서 state에 추가하여 반환하는 것
...
def generate_search_keyword(state:dict) -> dict:
    ...
    #dict형태 응답 -> 값 추출
    if isinstance(items, dict):
        keywords = [i for sub in items.values() for i in (sub if isinstance(sub, list) else [sub])]
...
#위의 모든 정보를 토대로 사용자에게 retun할 문장을 생성하는 함수이다.
def summarize_messages(state : dict) -> dict :
    ...
    #item => items리스트의 가장 첫번째 요소
    item = items[0]
    
    #state 상태에서필요한 정보를 추출
    season = state.get('seaseon', '')
    ...
    category = '음식' if intent == 'food' else ''
    
    #GPT 프롬포트 구성하기
    prompt = f'''
        현재 사용자는 {category}라는 의도로 질문하였습니다.
        계절은 {season}이고, 날씨는 {weather}, 시간은 {time_slot}입니다.
        
        추천할 아이템은 {category} : {item}이고,
        추천 장소는 {place_name}({place_address}),
        추천 장소의 지도 링크는 {place_url} 입니다.
        
        위의 정보를 바탕으로 사용자의 의도를 감성적이고 따듯한 시선으로 해석하여
    ...
    response = llm2.invoke([
        {'role' : 'system', 'content' : '너는 음식과 활동에 대해 추천할만한 재미있는 장소를 잘 알고있는 여향 보조 AI야.'},
        {'roke' : 'user', 'content' : prompt}
    ])
